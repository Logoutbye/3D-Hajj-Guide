import 'package:get/get.dart';

class Languages extends Translations {
  @override
  // TODO: implement keys
  Map<String, Map<String, String>> get keys => {
        // for eng
        'en_US': {
          'app_bar': 'Home Page',
          'language': 'English',
          'select_Language': 'Select Language.',
          'Ihram and Intentions': 'Step-1: Ihram and\n Intentions.',
          'Mina aka “City of tents”': 'Step-2: Mina aka “City of tents”',
          'Mina to Arafat': 'Step-3: Mina to Arafat',
          'Muzdalifah': 'Step-4: Muzdalifah',
          'Rami': 'Step-5: Rami– Stoning the devil',
          'Nahr': 'Step-6: Nahr',
          'Farewell Tawaf': 'Step-7: Farewell Tawaf',
          'step1': 'Step 1 – Ihram and Intentions:\n\n'
              'Making a pure intention and wearing the Ihram are the two of the first essential steps when going for Hajj. After making the niyat, Muslim pilgrims are advised to wear the Ihram- two pieces of unstitched white sheets for men and a loose-fitting Abaya for women, properly covering the whole body. It is recommended that the pilgrim should wear the Ihram on Dhul-Hijjah before entering Miqat – the outer boundaries of Makkah. The five entry points or relevant Miqats for pilgrims are as follows:\n\n'
              '• Abbyar Ali (Dhu’l Hulaifah) – This is the point of Miqat for pilgrims coming through Madina or from Saudi Arabia. They are advised to perform Hajj al-Tamatt’u.\n'
              '• (As-Sail Al-Kabeer) Qarn-al Manzil – This is the point of Miqat for pilgrims coming through or from Taif or Najd.\n'
              '• Al- Juhfah – Located near Rabigh, it is the point of Miqat for pilgrims coming through or from Egypt, Syria, or Morocco.\n'
              '• Dhat’Irq – Is the point of Miqat for pilgrims coming from or through Iraq.\n'
              '• Sa’adiyah (Yalamlam) – This is the point of Miqat for pilgrims coming through or from Yemen, India, or Pakistan.\n\n'
              'Also, once in Ihram, pilgrims are advised to recite Talbiyah while abstaining from all sinful acts. The Talibyah should be recited in a loud voice:\n\n'
              'لَبَّيْكَ اللَّهُمَّ لَبَّيْكَ، لَبَّيْكَ لَا شَرِيكَ لَكَ لَبَّيْكَ، انَّالْحَمْدَ، وَالنِّعْمَةَ، لَكَ وَالْمُلْكَ، لا شَرِيكَ لَكَ\n\n'
              'Labbayk Allaahumma labbayk, labbayk laa shareeka laka labbayk, ‘innal-hamda, wanni’mata, laka walmulk, laa shareeka laka.\n\n'
              '“Ever at Your service, O Allah, every at Your service. Ever at Your Service, You have no partner, ever at Your service. Verily all praise, blessings, and dominions are Yours. You have no partner.” (Muslim 2:841)\n\n'
              'After arriving at Makkah, Muslims intending to perform Hajj al-Tamatt’u should combine the rituals of Umrah with Hajj. For this, Muslims walk around the Holy Kaaba 7 times in an anti-clockwise direction, also known as Tawaf. They then perform the ritual of Hajr-e-Aswad (Black Stone) and move towards the next stage of Umrah, running between the Safa and Marwa hills. Once the Umrah is complete, pilgrims start their journey towards Mina.',
          'step2': 'Step 2 – Mina aka “City of Tents”:\n\n'
              'Situated 5 to 6 km from Makkah, Mina is a small town. Arriving at the tent city of Mina, pilgrims are advised to rest there until the following day. Starting with the noon prayer (Zuhr) and ending with the dawn prayer (Fajr), pilgrims recite all five Salahs while staying in Mina. Today, the land of Mina comprises modern tents that are equipped with all essential amenities. Muslims should recite both compulsory and non-compulsory prayers while staying at Mina.',
          'step3': 'Step 3 – Mina to Arafat, 9th Day of Dhul-Hijjah:\n\n'
              'On the morning of the second day of Hajj that is 9th Dhul-Hijjah, the pilgrims start walking towards Arafat while reciting Talbiyah at the top of their voices. Muslim pilgrims observe Zuhrain – a combination of Zuhr and Asr prayer with Qasar prayer upon reaching the mount of Arafat. This is known as Waquf – the act of standing before Allah (SWT) and is observed near the Jabal al-Rahmah from noon to sunset.\n\n'
              'Located 14.4 km from Mina, Mount Mercy or Jabal al Rahmah of the Arafat was where the beloved Prophet Muhammad (SAW) delivered the final sermon.',
          'step4': 'Step 4 – Muzdalifah:\n\n'
              'The pilgrims’ next destination for Hajj is Muzdalifah, a small town located between Mina and Mount Arafat. Upon arriving at sunset on the grounds of Muzdalifah, the pilgrims offer Maghribaen – a combined prayer of Maghrib and Isha. Muslims spend one whole night under the open sky and collect 49 pebbles of similar sizes for the ritual of Rami (stoning of the Devil). They then leave the town of Muzdalifah on the morning of 10th Dhul-Hijjah.\n\n'
              'Tawaf al-Ifadah and Sa’i:\n\n'
              'The pilgrims now head back to Makkah to perform Tawaf al-ifadah and Sa’I, the act of running between the Safa and Marwa hills. Once done, Muslims then go back to Mina to perform the acts of Rami, Nahr, and Halq.\n\n',
          'step5': 'Step 5 – Rami (Stoning the Devil):\n\n'
              'On arriving at Mina, pilgrims perform the act of Rami by stoning the Jamraat al-Aqabah. Seven stones are thrown at the column structure. The stoning of Jamrat is performed in the memory of the act of Prophet Ibrahim (AS) when the devil tried discouraging him from following Allah (SWT) command. In reply, Prophet Ibrahim (AS) threw small pebbles to make the devil go away. Rami should be carried out at noon each day. Rami is performed on the 11th and 12th of Dhul-Hijjah.',
          'step6': 'Step 6 – Nahr:\n\n'
              'After the completion of Rami, on 12th Dhul-Hijjah, Muslim pilgrims are advised to perform the sacrifice of an animal; it can be a camel or lamb. For this, pilgrims can either purchase sacrifice coupons or vouchers, stating that the sacrifice has been made on their behalf. The meat of the sacrificed animal should be distributed to the needy.'
              '\n\nHalq and Taqsir:\n\n'
              'Halq means shaving the head, while Taqsir means shortening or clipping of hair. After performing the sacred sacrifice, men pilgrims are advised to get their heads completely shaved or clipped. While forbidden to shave their heads, women are advised to get a strand or lock of their hairs chopped. The act of Halq and Taqsir symbolizes a Muslim’s complete devotion to Allah (SWT) and detachment from worldly appearances.',
          'step7': 'Step 7 – Farewell Tawaf:\n\n'
              'Muslim pilgrims making tawaf around the kaabaAfter completing the ritual, pilgrims return to the Holy Kaaba in Makkah to perform the “Tawaf al-Ifadah,” also known as the “Farewell Tawaf,” followed by Sa’I. Though this officially marks the end of Hajj, many pilgrims also visit Madinah before heading home.'
        },
        // for Urdu
        'ur_PK': {
          'app_bar': 'ہوم پیج',
          'language': 'اردو',
          'select_Language': 'زبان منتخب کریں۔',
          'Ihram and Intentions': 'پہلا قدم: احرام اور نیت۔',
          'Mina aka “City of tents”': 'دوسرا قدم: مینا عرف "خیموں کا شہر"',
          'Mina to Arafat': 'تیسرا قدم: منیٰ سے عرفات تک',
          'Muzdalifah': 'چوتھا قدم: مزدلفہ',
          'Rami': 'پانچواں قدم: رامی- شیطان کو سنگسار کرنا',
          'Nahr': 'چھٹا قدم: نہر',
          'Farewell Tawaf': 'ساتویں قدم: الوداعی طواف',
          'step1': 'مرحلہ 1 - احرام اور نیتیں:\n\n'
              'حج کے لیے جانے کے لیے خالص نیت کرنا اور احرام باندھنا دو اولین ضروری اقدامات ہیں۔ نعت کرنے کے بعد، مسلمان حجاج کو مشورہ دیا جاتا ہے کہ وہ احرام پہنیں- مردوں کے لیے بغیر سلی ہوئی سفید چادروں کے دو ٹکڑے اور خواتین کے لیے ڈھیلا ڈھالا عبایا، پورے جسم کو اچھی طرح ڈھانپیں۔ مستحب ہے کہ حجاج کو میقات یعنی مکہ کی بیرونی حدود میں داخل ہونے سے پہلے ذی الحجہ کو احرام باندھنا چاہیے۔ حجاج کے لیے پانچ داخلی مقامات یا متعلقہ میقات درج ذیل ہیں:\n\n'
              '• ابیار علی (ذوالحلیفہ) - یہ مدینہ کے راستے یا سعودی عرب سے آنے والے حاجیوں کے لیے میقات کا مقام ہے۔ انہیں حج التمتع کرنے کی تلقین کی جاتی ہے۔\n'
              '• (السائل الکبیر) قرن المنزل - طائف یا نجد سے آنے والے یا آنے والے زائرین کے لیے یہ میقات کا مقام ہے۔\n'
              '• الجوفہ - ربیع کے قریب واقع ہے، یہ مصر، شام یا مراکش سے آنے والے یا آنے والے زائرین کے لیے میقات کا مقام ہے۔\n'
              '• Dhat\'Irq - عراق سے یا اس کے راستے آنے والے زائرین کے لیے میقات کا نقطہ ہے۔\n'
              '• سعدیہ (یلملم) - یہ یمن، ہندوستان یا پاکستان سے آنے والے یا آنے والے حاجیوں کے لیے میقات کا نقطہ ہے۔\n\n'
              'نیز، احرام میں ایک بار، حجاج کو تلبیہ پڑھنے کی تلقین کی جاتی ہے اور تمام گناہوں سے پرہیز کیا جاتا ہے۔ تلبیہ اونچی آواز میں پڑھی جائے:\n\n'
              'لَبَّيْكَ اللَّهُمَّ لَبَّيْكَ، لَبَّيْكَ لَا شَرِيكَ لَكَ لَبَّيْكَ، انَّالْحَمْدَ، وَالنِّعْمَةَ، لَكَ وَالْمُلْكَ، لا شَرِيكَ لَكَ\n\n'
              ' لبیک اللھم لبیک، لبیک لا شریکا لکا لبیک، انل حمدہ، ونیومتہ، لکا والملک، لا شریکا لکا۔\n\n'
              'ہر وقت تیری خدمت میں اے اللہ ہر تیری خدمت میں۔ کبھی آپ کی خدمت میں، آپ کا کوئی شریک نہیں، کبھی آپ کی خدمت میں۔ بے شک تمام تعریفیں، نعمتیں اور بادشاہتیں تیری ہی ہیں۔ تمہارا کوئی شریک نہیں ہے۔‘‘ (مسلم 2:841)\n\n'
              'مکہ پہنچنے کے بعد حج تمتع کرنے کا ارادہ رکھنے والے مسلمانوں کو عمرہ کے مناسک کو حج کے ساتھ ملانا چاہیے۔ اس کے لیے مسلمان خانہ کعبہ کے گرد 7 بار گھڑی کی مخالف سمت میں چلتے ہیں، جسے طواف بھی کہا جاتا ہے۔ اس کے بعد وہ حجر اسود (سیاہ پتھر) کی رسم ادا کرتے ہیں اور صفا اور مروہ کی پہاڑیوں کے درمیان دوڑتے ہوئے عمرہ کے اگلے مرحلے کی طرف بڑھتے ہیں۔ عمرہ مکمل ہونے کے بعد زائرین منیٰ کی طرف سفر شروع کر دیتے ہیں۔\n\n',
          'step2': 'مرحلہ 2 - مینا عرف "خیموں کا شہر":\n\n'
              'مکہ مکرمہ سے 5 سے 6 کلومیٹر کے فاصلے پر واقع منیٰ ایک چھوٹا سا شہر ہے۔ خیمہ کے شہر منیٰ میں پہنچ کر عازمین کو اگلے دن تک وہاں آرام کرنے کا مشورہ دیا جاتا ہے۔ ظہر کی نماز (ظہر) سے شروع ہو کر فجر کی نماز (فجر) پر ختم ہو کر، حجاج منیٰ میں قیام کے دوران پانچوں نمازیں پڑھتے ہیں۔ آج منیٰ کی سرزمین جدید خیموں پر مشتمل ہے جو تمام ضروری سہولیات سے آراستہ ہیں۔ مسلمانوں کو منیٰ میں قیام کے دوران لازمی اور غیر لازمی دونوں نمازیں پڑھنی چاہئیں۔',
          'step3': 'مرحلہ 3 - منی سے عرفات تک، 9 ذوالحجہ کا دن:\n\n'
              'حج کے دوسرے دن یعنی 9 ذی الحجہ کی صبح حجاج اپنی آواز کی بلندی پر تلبیہ پڑھتے ہوئے عرفات کی طرف چلنا شروع کر دیتے ہیں۔ مسلمان حجاج ظہرین کا مشاہدہ کرتے ہیں - عرفات کی پہاڑی پر پہنچنے پر نماز ظہر اور عصر کی نماز کے ساتھ قصر کی نماز۔ اسے وقوف کہا جاتا ہے - اللہ (SWT) کے سامنے کھڑے ہونے کا عمل اور جبل الرحمہ کے قریب دوپہر سے غروب آفتاب تک دیکھا جاتا ہے۔\n\n'
              'منی سے 14.4 کلومیٹر کے فاصلے پر عرفات کا پہاڑ رحمت یا جبل الرحمہ تھا جہاں پیارے نبی محمد صلی اللہ علیہ وسلم نے آخری خطبہ دیا۔',
          'step4': ':مرحلہ 4 - مزدلفہ:\n\n'
              'حج کے لیے عازمین کی اگلی منزل مزدلفہ ہے، جو منیٰ اور کوہ عرفات کے درمیان واقع ایک چھوٹا سا شہر ہے۔ مزدلفہ کے میدان میں غروب آفتاب پر پہنچنے پر، حجاج مغرب اور عشاء کی مشترکہ نماز مغربین ادا کرتے ہیں۔ مسلمان پوری ایک رات کھلے آسمان تلے گزارتے ہیں اور رامی (شیطان کو سنگسار) کی رسم کے لیے اسی سائز کے 49 کنکریاں جمع کرتے ہیں۔ اس کے بعد وہ 10 ذی الحجہ کی صبح مزدلفہ شہر سے نکلے۔'
              'طواف افاضہ اور سعی:\n\n'
              'حجاج اب طواف الافادہ اور سعی کرنے کے لیے مکہ واپس چلے جاتے ہیں، جو صفا اور مروہ کی پہاڑیوں کے درمیان چلنے کا عمل ہے۔ ایک بار مکمل ہوجانے کے بعد، مسلمان پھر منیٰ واپس چلے جاتے ہیں تاکہ رمی، نہر اور ہلق کے اعمال انجام دیں۔\n\n',
          'step5': 'مرحلہ 5 - رامی (شیطان کو سنگسار کرنا):\n\n'
              ' منیٰ پہنچ کر حجاج کرام نے جمرات العقبہ کو سنگسار کرکے رمی کا عمل انجام دیا۔ کالم کے ڈھانچے میں سات پتھر پھینکے گئے ہیں۔ جمرات کو سنگسار کرنا حضرت ابراہیم (ع) کے اس عمل کی یاد میں کیا جاتا ہے جب شیطان نے انہیں اللہ کے حکم پر عمل کرنے سے باز رکھنے کی کوشش کی تھی۔ جواب میں حضرت ابراہیم علیہ السلام نے شیطان کو دور کرنے کے لیے چھوٹی چھوٹی کنکریاں پھینکیں۔ بھنگ کو ہر روز دوپہر کے وقت کیا جانا چاہئے۔ رمی 11 اور 12 ذوالحجہ کو ادا کی جاتی ہے۔',
          'step6': 'مرحلہ 6 - نحر:\n\n'
              'رمی کی تکمیل کے بعد، 12 ذی الحجہ کو، مسلمان حجاج کو جانور کی قربانی کرنے کی تلقین کی جاتی ہے۔ یہ اونٹ یا بھیڑ کا بچہ ہو سکتا ہے۔ اس کے لیے حجاج یا تو قربانی کے کوپن یا واؤچر خرید سکتے ہیں، یہ بتاتے ہوئے کہ قربانی ان کی طرف سے کی گئی ہے۔ قربانی کے جانور کا گوشت ضرورت مندوں میں تقسیم کیا جائے۔\n\n'
              'حلق اور تقصیر:\n\n'
              'حلق کا مطلب ہے سر منڈوانا، جب کہ تقسیر کا مطلب ہے بالوں کو چھوٹا کرنا یا تراشنا۔ مقدس قربانی کرنے کے بعد، حجاج کے مردوں کو مشورہ دیا جاتا ہے کہ وہ اپنے سر کو مکمل طور پر منڈوائیں یا تراشیں۔ اگرچہ سر منڈوانا منع ہے، لیکن خواتین کو مشورہ دیا جاتا ہے کہ وہ اپنے کٹے ہوئے بالوں کو بند کر لیں۔ حق اور تقسیر کا عمل ایک مسلمان کی اللہ سبحانہ وتعالیٰ سے مکمل عقیدت اور دنیاوی نمود و نمائش سے لاتعلقی کی علامت ہے۔',
          'step7': 'مرحلہ 7 - الوداعی طواف:\n\n'
              'مسلم زائرین خانہ کعبہ کا طواف کرتے ہیں رسم مکمل کرنے کے بعد، حجاج کرام مکہ مکرمہ میں خانہ کعبہ کی طرف "طواف الافادہ، جسے "الوداعی طواف" بھی کہا جاتا ہے، انجام دینے کے لیے واپس آتے ہیں، جس کے بعد سعی ہوتی ہے۔ اگرچہ یہ سرکاری طور پر حج کے اختتام کی نشاندہی کرتا ہے، لیکن بہت سے حجاج گھر جانے سے پہلے مدینہ بھی جاتے ہیں۔'
        },
      };
}
